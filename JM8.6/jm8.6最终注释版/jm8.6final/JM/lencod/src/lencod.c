...
int main(int argc,char **argv)
{
  ...
  Configure (argc, argv);  //chj 配置参数

  AllocNalPayloadBuffer();  //chj 分配NAL空间

  init_poc();  //chj 初始化POC－picture order count 
  ...
  if (input->PicInterlace != FRAME_CODING) //chj 如果是场编码
  {
    top_pic = malloc_picture();//chj 分别给顶场底场分配空间
    bottom_pic = malloc_picture();
  }
  init_rdopt (); //chj RDO 模式选择的参数
  ...
  if(input->FMEnable)	// 决定快速运动估计算法使用与否。1：使用，0：全搜索
    DefineThreshold();
    ...
    if (img->type == I_SLICE)
    {
      if(input->RCEnable)
      {
        if (input->intra_period == 0)
        {
          ...
          /* number of P frames */
          np = input->no_frames-1;  //chj no_frames不包含B帧
