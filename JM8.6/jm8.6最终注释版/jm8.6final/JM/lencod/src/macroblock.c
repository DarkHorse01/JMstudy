...
void set_MB_parameters (int mb_addr)
{
  ...
  img->pix_x   = img->block_x << 2;  //宏块原点的像素坐标
                               ...
                               StorablePicture **list)
{
  ...
  //chj a 这里利用mv，找到参考块的坐标
  pel_t (*get_pel) (pel_t**, int, int, int, int) = UMVPelY_14;
  //chj a UMVPelY_14用在靠近图像边沿时使用，
  //chj 这时候需要判定是否出界,FastPelY_14是在图像内部中使用，
  //chj 无需判定边界条件
  ...
  *mpred++ = get_pel (ref_pic, j3, i3, img_height, img_width);
  //chj 因为上层调用的时候已经确定了使用前后哪个参考帧
  //chj 这里直接获取其像素值，注意mv是以四分之一像素为单位的
}
 ...
 ************************************************************************
 * \brief
 *    Predict one 4x4 Luma block，得到mpr，即current best prediction mode?
       mpr里面存的是参考块的像素
 ************************************************************************
 */
void
  ...
  for (byy=0, block_y=mb_y; block_y<mb_y+8; byy+=4, block_y+=4)
  {
    ...
    for (bxx=0, block_x=mb_x; block_x<mb_x+8; bxx+=4, block_x+=4)
    {
      ...
      for (i=0; i<4; i++)
      {
		...
		//chj a is m7 is used as a temporary variable for processing residue data
      }

      //===== DCT, Quantization, inverse Quantization, IDCT, Reconstruction =====      
      if (img->NoResidueDirect != 1 && !skipped  )//chj 非skip模式，残差不为0
      {
        //===== DCT, Quantization, inverse Quantization, IDCT, Reconstruction =====
		//chj 变换量化反量化重建全部在dct一个函数中
		//chj 注意dct函数的功能实现都是基于全局变量
...
void ChromaResidualCoding (int* cr_cbp)
{
  ...
  for (*cr_cbp=0, uv=0; uv<2; uv++)
  {
    ...
    {
	  // 设置色度块的参数，包括:4个4*4块的分割模式和预测方向(p_dir, fw_mode, bw_mode)、4个4*4块的参考帧索引（refframe, bw_ref）
      ...
      if (skipped && img->type==SP_SLICE)
      {
        ...
        else
        {
          for (j=0; j<8; j++)
            for (i=0; i<8; i++)
            {
              img->m7[i][j] = imgUV_org[uv][img->opix_c_y+j][img->opix_c_x+i] - img->mpr[i][j];	// 计算色度残差
            }
        }
    ...
    else
    {
      if (!img->NoResidueDirect && !skipped)
      {
        if (img->type!=SP_SLICE || IS_INTRA (&img->mb_data[img->current_mb_nr]))
          *cr_cbp=dct_chroma   (uv,*cr_cbp);	// 色度块变换、量化、反量化、反变换，并保存重构值
        else
          *cr_cbp=dct_chroma_sp(uv,*cr_cbp);
      }
    }
...
void IntraChromaPrediction8x8 (int *mb_up, int *mb_left, int*mb_up_left)
{
  ...
  for (uv=0; uv<2; uv++)
  {
    ...
    // DC prediction	// 将 8*8 色度块划分成 4 个 4*4 区域进行预测，参见标准：8.3.4.1 小节
    for (block_y=0; block_y<8; block_y+=4)
    ...
    // vertical prediction	// 参见标准：8.3.4.3 小节
    if (mb_available_up)
    {
      for (i=0; i<8; i++)
    ...
    // horizontal prediction	// 参见标准：8.3.4.2 小节 
    ...
    // plane prediction	// 参见标准：8.3.4.4 小节 
...
int writeCBPandLumaCoeff ()
{
  ...
  if (!IS_NEWINTRA (currMB))  //不是I16MB类型
  {
    //=====  L U M I N A N C E   =====
    //--------------------------------
    for (i=0; i<4; i++)  if (cbp & (1<<i)) // 后面4位每一位代表一个8x8块
    {
...
int writeCoeff4x4_CAVLC (int block_type, int b8, int b4, int param)
{
  ...
  currSE->value1 = numcoeff; //chj 非0系数总个数记录在value1
  currSE->value2 = numtrailingones; //chj 拖尾系数个数记录在value2
...
int find_sad_16x16(int *intra_mode)
{
  ...
  for (k=0;k<4;k++)
  {
    ...
    else
    {
      ...
      for (j=0;j<4;j++)
        for (i=0;i<4;i++)
          M4[i][j]=M0[0][i][0][j]/4;	// 由于SATD变换不是归一化矩阵，变换后的系数值幅值增加，因此要相应地/4
  ...
  best_intra_sad2 = best_intra_sad2/2;	// 由于SATD变换不是归一化矩阵，变换后的系数值幅值增加，因此要相应地/2
