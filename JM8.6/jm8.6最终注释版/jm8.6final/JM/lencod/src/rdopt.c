...
{
  int mx,my,y,x,i,j;
  //如果是I_slice,则每个像素的pixel_map全部为1
  if (img->type==I_SLICE)
  {
 ...
 *    如果一个给定区域在紧接着的下一帧有一个帧内刷新的动作，那么这个区域都是不可用的
 *
 *************************************************************************************
 */
...
int Mode_Decision_for_4x4IntraBlocks (int  b8,  int  b4,  double  lambda,  int*  min_cost)
{
  ...
  double  rdcost;
  int     block_x     = 8*(b8%2)+4*(b4%2);	//当前4x4块左上角像素相对本宏块左上角像素的横坐标（单位是像素）
  int     block_y     = 8*(b8/2)+4*(b4/2);	       //当前4x4块左上角像素相对本宏块左上角像素的纵坐标（单位是像素）
  int     pic_pix_x   = img->pix_x+block_x;	//当前4x4块左上角像素相对本图像左上角像素的横坐标（单位是像素）
  int     pic_pix_y   = img->pix_y+block_y;	//当前4x4块左上角像素相对本图像左上角像素的纵坐标（单位是像素）
  int     pic_opix_x   = img->opix_x+block_x;   //在帧模式时等于pic_pix_x
  int     pic_opix_y   = img->opix_y+block_y;   //在帧模式时等于pic_pix_y
  ...
  getLuma4x4Neighbour(img->current_mb_nr, block_x/4, block_y/4, -1,  0, &left_block);	// 获取当前4*4块左侧相邻4*4块的信息（包括所在宏块地址、可用性、坐标）
  getLuma4x4Neighbour(img->current_mb_nr, block_x/4, block_y/4,  0, -1, &top_block);	// 获取当前4*4块上方相邻4*4块的信息（包括所在宏块地址、可用性、坐标）
  ...
  *min_cost = (1<<20);
  ...
  //分别计算当前4x4块在9种intra4x4帧内预测模式下的预测块，参见标准：8.3.1.2 小节
...
int Mode_Decision_for_8x8IntraBlocks(int b8,double lambda,int *cost)
{
  ...
  *cost = (int)floor(6.0 * lambda + 0.4999);	//floor--向下取整,ceil--向上取整

  for (b4=0; b4<4; b4++)
  {
    if (Mode_Decision_for_4x4IntraBlocks (b8, b4, lambda, &cost4x4))	// 对8*8块中的4个4*4的块循环执行
    {
      nonzero        = 1;//只要该8x8块有一个4x4块有非零量化系数nonzero即为1
    }
    *cost += cost4x4;
  }

  return nonzero;//返回该8x8块的量化系数是否有非零值
}
...
*     在intra4x4的9种模式中选择最佳模式并计算代价，返回宏块CBP
 *************************************************************************************
 */
...

{
  ...
  {
    if (Mode_Decision_for_8x8IntraBlocks (b8, lambda, &cost8x8))//对16x16块中的4个8x8的块循环执行
    {
      cbp |= (1<<b8);//根据每个8x8块的量化后系数是否为零给cbp赋值
    }
    *cost += cost8x8;
  }

  return cbp;//返回该宏块的cbp
}
                             ...
                             int     bwd_ref)    // <-- abp type
{
  ...
  //=====
  //----- block 8x8 mode -----
  if (input->symbol_mode == UVLC)//熵编码模式
  {
...
Intra16x16_Mode_Decision (Macroblock* currMB, int* i16mode)
{
  ...
  // 对每种预测模式残差进行 Hadamard 变换，对 DC 系数组成的块再进行一次 Hadamard 变换。将变换后
  // 的残差值取绝对值并求和（即SAD）作为该种预测模式的代价。将代价最小的预测模式作为最优模式。
  find_sad_16x16 (i16mode);   /* get best new intra mode */
  
  // 计算 intra16*16 最优模式下的亮度重构值（包括变换、量化、反量化、反变换）
 ...
 *************************************************************************************
 * \brief
 *    R-D Cost for a macroblock 计算一个宏块的代价
 *************************************************************************************
 */
                        ...
                        double*  min_rdcost)  // <-> minimum rate-distortion cost
{
  ...
  int         bframe    = (img->type==B_SLICE);
  int         tmp_cc;
  int         use_of_cc =  (img->type!=I_SLICE &&  input->symbol_mode!=CABAC);	// 该变量表示码流中存在语法元素 mb_skip_run
  int         cc_rate, dummy;
  
  //=====
  //=====  SET REFERENCE FRAMES AND BLOCK MODES
  //=====
  SetModesAndRefframeForBlocks (mode);	// 与上层该函数重复
  ...
  img->i16offset = 0;
  dummy = 0;
  ChromaResidualCoding (&dummy);	// 计算色度重构值（包括残差计算、量化、变换、反变换、反量化）
  ...
  //-------------------------------------------------------
  reset_coding_state (cs_cm);	// 恢复编码前的状态
 ...
 */
 void encode_one_macroblock ()
 {
   ...
   //DO NOT CHANGE ORDER !!!	亚宏块分割模式
   //0：Direct_8*8，4：8*8，5：8*4，6：4*8，7：4*4 
   ...
   //fw_cost指foreward cost, bw_mcost是backward cost,bid_mcost是bidirection cost
   //所以，当没有bframe的时候，就只有fw_cost在用
   ...
   //===== SET VALID MODES =====
   valid[I4MB]   = 1;	//宏块默认都可以采用intra4x4模式编码
   valid[I16MB]  = 1;	//宏块默认都可以采用intra16x16模式编码

   //valid[0]是inter模式的总开关
   valid[0]      = (!intra );	//宏块是否采用inter模式编码
   valid[1]      = (!intra && input->InterSearch16x16);	//宏块是否采用inter16x16模式编码
   valid[2]      = (!intra && input->InterSearch16x8);	//宏块是否采用inter16x8模式编码
   valid[3]      = (!intra && input->InterSearch8x16);	//宏块是否采用inter8x16模式编码
   valid[4]      = (!intra && input->InterSearch8x8);	//宏块是否采用inter8x8模式编码
   valid[5]      = (!intra && input->InterSearch8x4);	//宏块是否采用inter8x4模式编码
   valid[6]      = (!intra && input->InterSearch4x8);	//宏块是否采用inter4x8模式编码
   valid[7]      = (!intra && input->InterSearch4x4);	//宏块是否采用inter4x4模式编码
   valid[P8x8]   = (valid[4] || valid[5] || valid[6] || valid[7]);	//宏块是否采用亚宏块模式编码
   valid[12]     = (siframe);	//宏块是否采用SI模式编码

   if (!img->MbaffFrameFlag)//非帧场自适应
   {
   ...
   //runs=2是针对loss rdo模式，其余的情况runs=1
   for (rerun=0; rerun<runs; rerun++)	
   {
     ...
     if (!intra) //如果没有被强制转为帧内模式
     {
       //===== set direct motion vectors =====
       if (bframe)  //如果是B帧，baseline无B帧
       {
         Get_Direct_Motion_Vectors ();
       }
       
       //===== MOTION ESTIMATION FOR 16x16, 16x8, 8x16 BLOCKS =====       
       for (min_cost=1<<20, best_mode=1, mode=1; mode<4; mode++)//mode从1到3
       {  
         if (valid[mode])
         {
           for (cost=0, block=0; block<(mode==1?1:2); block++)
           {
             //宏块分割运动估计(16*16、16*8、8*16)
             //对所有参考帧遍历(该遍历在PartitionMotionSearch函数内)，对所有宏块分割模式遍历
             //对每个宏块,如果所有宏块分割模式都可用,则下面的函数执行5次: 1次16x16，2次16x8，2次8x16
             PartitionMotionSearch (mode, block, lambda_motion);	

             //--- set 4x4 block indizes (for getting MV) ---
             j = (block==1 && mode==2 ? 2 : 0);	//无用代码
             i = (block==1 && mode==3 ? 2 : 0);	//无用代码
             //此处赋值的j和i在没使用前就会被改变
          ...
          if (cost < min_cost)
          {
            best_mode = mode;//记录目前为止的最佳模式类型
            min_cost  = cost;
          }
        } // if (valid[mode])
      } // for (mode=1; mode<4; mode++)

       //上面估计了宏块分割的三种模式，现在开始看亚宏块分割能不能更好
      if (valid[P8x8]) //如果允许亚宏块编码
      {
        cost8x8 = 0;  //记录亚宏块分割的代价
        
        //===== store coding state of macroblock =====
        store_coding_state (cs_mb);//csobj类型记录宏块编码状态
        
        //=====  LOOP OVER 8x8 SUB-PARTITIONS  (Motion Estimation & Mode Decision) =====
        /*b 求每个8x8块的代价,而且记录下CBP*/
        for (cbp8x8=cbp_blk8x8=cnt_nonz_8x8=0, block=0; block<4; block++)
        {
          //--- set coordinates ---
          //i0 j0是像素坐标,i1 j1是4x4块坐标
          ...
          {
            //b8_mode_table[6]上面定义为{0,4,5,6,7}
            if (valid[mode=b8_mode_table[index]])//判断亚宏块模式分割是否可用，注意这里先赋值再判断
            {
              curr_cbp_blk = 0;  //初始化当前块的cbp
              
              if (mode==0) //采用direct模式，当不是bframe时，mode从4到7
              {
                //--- Direct Mode ---
                if (!input->rdopt)
                {
                  cost_direct += (cost = Get_Direct_Cost8x8 ( block, lambda_mode ));
                  if (cost==1<<30) //代表不能用direct模式
                    cost_direct = (1<<30);
                  have_direct ++; 
                }
              ...
              else  //对亚宏块分割进行运动估计算出代价
              {
                //--- motion estimation for all reference frames ---
                //对所有参考帧遍历，对所有亚宏块分割模式遍历
                //MB分为4个8x8块:block0, block1, block2, block3
                //对每个8x8块继续分割为8x4, 4x8, 4x4,依次遍历
                //因此，对每个宏块,如果所有亚宏块分割模式都可用,则下面的函数执行16次
                PartitionMotionSearch (mode, block, lambda_motion);
			  //运动搜索计算cost，结果放在motion_cost 中
                ...
                for (fw_mcost=max_mcost, ref=0; ref<listXsize[LIST_0+list_offset]; ref++)
                {
                  if (!checkref || ref==0 || CheckReliabilityOfRef (block, LIST_0, ref, mode))
                  {
                    ...
                    mcost += motion_cost[mode][LIST_0][ref][block];//一个宏块的cost
                    if (mcost < fw_mcost)
                    {
                      fw_mcost    = mcost;  //注意: cost赋给fw_mcost
                      best_fw_ref = ref;
                    }
                  }
                }
              ...
              {
                min_cost8x8                  = cost; //chj 存入min_cost8x8
                min_rdcost                   = rdcost;
...
//          diffy[j][i] = imgY_org[img->opix_y+j][img->opix_x+i]-img->mpr[j][i];
        ...
        if(cost8x8 < min_cost)  //如果比min_cost小就取代之前的宏块分割模式
        {
           best_mode = P8x8;//记录目前为止的最佳模式类型
           min_cost = cost8x8;
        }
      }
      else //对应if (valid[P8x8])，如果不允许亚宏块编码
      {
        cost8x8 = (1<<20);  //赋个很大的值，后面模式选择的时候将不会选亚宏块模式
      }
    ...
    {
      min_cost = (1<<20);//赋个很大的值，后面模式选择的时候将不会选inter模式
    }
    ...
    else
    {
      ...
      {
        /*在intra4x4的9种模式中选择最佳模式并计算代价，返回宏块CBP
        input->rdopt为1时，在RDCost_for_macroblocks中调用，
        否则，单独计算如下
      */
        currMB->cbp = Mode_Decision_for_Intra4x4Macroblock (lambda_mode, &cost);
        if (cost <= min_cost) //如果cost小于目前最小的cost
        {
          //Rate control
          for (i=0; i<16; i++)
          for(j=0; j<16; j++)
          {
            //保存采用intra4*4编码时最佳模式的残差块,img->mpr是最佳模式的预测块数据
            diffy[j][i] = imgY_org[img->pix_y+j][img->pix_x+i]-img->mpr[i][j];	
          }            
          min_cost  = cost;
          best_mode = I4MB;//记录目前为止的最佳模式类型
        }
      }
      if (valid[I16MB]) // check INTRA16x16
      {
        /*可以看到在实现上，计算intra4x4的9种预测模式时，所有的工作都在
        Mode_Decision_for_Intra4x4Macroblock这个函数之中完成了，
        而计算intra16x16的4种预测模式时，分别调用了几个函数来完成
      */
        //intrapred_luma_16x16是计算当前宏块在4种intra16*16帧内预测模式下的预测块
        intrapred_luma_16x16 ();	
        //计算intra16*16类型的代价（以SATD作为判断标准，因为该段代码是不采用RDO的处理过程）
        ...
        if (cost < min_cost)//如果cost小于目前最小的cost
        {
          ...
          for (i=0; i<16; i++)
            ...
            best_mode   = I16MB;//记录目前为止的最佳模式类型
            //对当前宏块采用intra16*16类型进行编码，并计算CBP
            currMB->cbp = dct_luma_16x16 (i16mode);			
        }
      }
    }
