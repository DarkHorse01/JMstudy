...
typedef enum
{
  //标准的annex B标题就叫Byte stream format
...
{
  int available;  //像素所在宏块的可用性
  int mb_addr;  //宏块地址
  int x;        //宏块中的相对坐标(单位:像素)
  int y;
  int pos_x;  //像素绝对坐标
  int pos_y;
} PixelPos;
...
//! Syntaxelement
typedef struct syntaxelement//各种愈发元素的统一，抽象表示
{
  int                 type;           //!< type of syntax 语法 element for data part.
  int                 value1;         //!< numerical 算术 value of syntax element
  int                 value2;         //!< for blocked symbols, e.g. run/level
  int                 len;            //!< length of code
  int                 inf;            //!< info part of UVLC code
  unsigned int      bitpattern;//指数哥伦布编码的比特表示
...
typedef struct macroblock
{
  ...
  int                 bitcounter[MAX_BITCOUNTER_MB];	// 参见 BitCountType 枚举定义
  ...
  int                 cbp ;
  int                 cbp_blk ;    //!< 1 bit set for every 4x4 block with coefs (not implemented 实施 for INTRA)
  int                 b8mode[4];
  int                 b8pdir[4];
...
typedef struct
{
  ...
  byte            *streamBuffer;      //输入缓存区 以byte为单位
  //chj 用来保存最终的编码结果，输出到test.264
...
typedef struct   //chj 刚好100个成员
{
  ...
  int qp02;
#endif
  int rdopt;//RDO码率失真化   在Rdopt.c   1831行
#ifdef _LEAKYBUCKET_
...
typedef struct
{
  ...
  unsigned PicSizeInMbs;//chj 图像中MB个数
...
//  int delta_pic_order_cnt[2];
  ...
  /*rate control*/
  int NumberofHeaderBits;	// 图像中所有基本单元头比特数之和
  int NumberofTextureBits;	// 图像中所有基本单元纹理比特数之和
  int NumberofBasicUnitHeaderBits;	// 图像中前一个编码基本单元所用头比特数，该变量在帧级 RC 时候不会被使用
  int NumberofBasicUnitTextureBits;	// 图像中前一个编码基本单元所用纹理比特数，该变量在帧级 RC 时候不会被使用
  double TotalMADBasicUnit;		// 一个基本单元中所有宏块 MAD 之和
  ...
  int bot_MB;                // 是否是bottom MB
  int write_macroblock_frame;

  int DeblockCall;       //是否宏块已经被deblock过了
