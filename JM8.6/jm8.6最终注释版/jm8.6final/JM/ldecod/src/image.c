 ...
 *    解码一帧的过程中，需要对参考列表进行初始化、重排序以及参考图像的标记三步
 ***********************************************************************
 */
...
int decode_one_frame(struct img_par *img,struct inp_par *inp, struct snr_par *snr)
{
  ...
  {
    current_header = read_new_slice();  //从码流中读取一帧新的slice
 ...
 ************************************************************************
 * \brief
 *    Reads new slice from bit_stream  从比特流中读取新片段
 ************************************************************************
 */
int read_new_slice()
{
...
//  int i;
  ...
  while (1)
  {
    ...
    if (input->FileFormat == PAR_OF_ANNEXB)
      ret=GetAnnexbNALU (nalu);                           //从文件流中读取nalu单元
    else
      ret=GetRTPNALU (nalu);
...
//      printf ("read_new_slice: returning %s\n", "EOS");
    ...
    switch (nalu->nal_unit_type)
    {
      ...
      case NALU_TYPE_IDR:
        ...
        BitsUsedByHeader = FirstPartOfSliceHeader();        //读取文件头的第一部分
        ...
        init_lists(img->type, img->currentSlice->structure);  //参考帧列表初始化
        reorder_lists (img->type, img->currentSlice);         //参考帧列表修改
...
void exit_picture()
{
  ...
  store_picture_in_dpb(dec_picture);    //把解码完成的图像保存到解码缓存区中
  dec_picture=NULL;
