...
void init_lists(int currSliceType, PictureStructure currPicStructure)
{
  ...
  if (currPicStructure == FRAME)  
  {
    for (i=0; i<dpb.ref_frames_in_buffer; i++)  //!< 遍历dpb中所有的参考帧（包括短期和长期参考帧）  
    {
      ...
      {
        if ((dpb.fs_ref[i]->frame->used_for_reference)&&(!dpb.fs_ref[i]->frame->is_long_term)) //如果参考帧是被用作参考，并且并不是长期参考
        {
          if( dpb.fs_ref[i]->frame_num > img->frame_num )  //如果参考帧的frame_num>当前读出来这一帧的frame_num
          {
...
static void idr_memory_management(StorablePicture* p)
{
  ...
  if (p->no_output_of_prior_pics_flag)
  {
    ...
    for (i=0; i<dpb.used_size; i++)
    { //当前参考帧缓存中的所有图像并不会写出到输出的码流中去，而是会被直接抛弃掉
  ...
  {  //否则进行flush_dpb的操作，将dpb中的图像写出到输出的图像中去
    flush_dpb();
  }
  dpb.last_picture = NULL;
  ...
  if (p->long_term_reference_flag)   //如果当前的idr帧被设置为长期参考帧
  {
    dpb.max_long_term_pic_idx = 0;
    p->is_long_term           = 1;    //表示当前帧作为长期参考
...
static void sliding_window_memory_management(StorablePicture* p)
{
  ...
  // 当前的dpb中已经保存的参考帧，等于除过长期参考帧之外已经达到上限，就进行一次清理
  if (dpb.ref_frames_in_buffer==dpb.num_ref_frames - dpb.ltref_frames_in_buffer)
  {
    for (i=0; i<dpb.used_size;i++)
    {
      if (dpb.fs[i]->is_reference  && (!(dpb.fs[i]->is_long_term)))
      {  //选取索引值最小的，又不用做长期参考帧的（第一个短期参考帧）
        unmark_for_reference(dpb.fs[i]);    //设置不作为参考
        update_ref_list();                  //并且更新参考帧列表
        break;                              //结束
      }
    }
  }

  p->is_long_term = 0;                       //同时，新进来的这一帧会被作为短期参考帧保存
}
  ...
  for (i=0; i<dpb.ref_frames_in_buffer; i++)
  {
    if (p->structure == FRAME)
    {
      if ((dpb.fs_ref[i]->is_reference==3) && (dpb.fs_ref[i]->is_long_term==0))
      {
        ...
        {
          unmark_for_reference(dpb.fs_ref[i]);   //标记为不做为参考
          return;
        }
      }
    }
...
void store_picture_in_dpb(StorablePicture* p)
{
  ...
  if (p->idr_flag)                //如果当前图像是idr帧
    idr_memory_management(p);     //idr内存管理
  else
  {                               //非idr帧
    // adaptive memory management //自适应内存控制
    // 如果标志位为真，且作为参考
    if (p->used_for_reference && (p->adaptive_ref_pic_buffering_flag))
      adaptive_memory_management(p);   //自适应内存控制
  }
  ...
  // sliding window, if necessary    滑动窗口
  if ((!p->idr_flag)&&(p->used_for_reference && (!p->adaptive_ref_pic_buffering_flag)))
  {  //如果当前帧不是idr帧，并且当前帧会用作参考帧，以及adaptive_标志位为假
